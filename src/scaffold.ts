...
// Emits Seo.svelte. Note the double-escaping in the jsonLdScript line below: this
// is a JS string that emits a Svelte file which re-escapes at runtime. `<\/script>`
// (escaped slash) stops the literal from closing this component's own <script> block,
// and `\\u003c` becomes the runtime `<` that escapes `<` in the JSON-LD payload.
...
// Emits the sitemap endpoint. The regex literals below are double-escaped because
// they live inside a JS template literal that emits a .ts file (e.g. `\\/\\+page`
// in source becomes `/\+page` in the generated route).
